...
- `src/common.js`, `src/types.js`, `src/theme.js`, `src/theme-toggle.js`, `src/styles.css`
...
- `src/*.html`
  ...
  - Modifiable: **Yes**.
  - Constraints: keep page IDs/entrypoint expectations used by the extension pages.
- `manifest.json`
...
- `public/background.js`, `public/assets/`
  - Runtime extension assets loaded directly from source.
  - Modifiable: **Yes**.
  - Constraints: preserve manifest-referenced paths unless the task explicitly changes packaging.
...
- `vendor/`
  - Vendored third-party runtime modules.
  - Modifiable: **Yes, when dependency/runtime tasks require it**.
  - Constraints: keep imports local and avoid introducing remote runtime dependencies.
...
- Load the repository directly as an unpacked extension after code changes affecting runtime behavior or packaging.
...
- For important user-visible or runtime behavior changes, prepare the appropriate version bump before creating and pushing a release tag unless a human requests otherwise.
- Do not edit version fields manually; use `npm version` so `package.json`, `package-lock.json`, and the git tag stay synchronized, and keep `manifest.json` aligned via the sync script.
...
- [ ] The repository still loads directly as an unpacked extension when runtime/package behavior changed.
